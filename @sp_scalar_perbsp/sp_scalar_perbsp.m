...
function sp = sp_scalar_perbsp (knots, degree, msh, transform)

  
  transform = 'grad-preserving';
 

  sp.space_type = 'spline'; %TRY: making it spline type
...
% For the 1D case
  ...
  nodes = msh.qn;
  for idim = 1:msh.ndim
    sp.sp_univ(idim) = sp_perbsp_1d_param (knots{idim}, degree(idim), nodes{idim}, 'gradient', true, 'hessian', true);
  end
