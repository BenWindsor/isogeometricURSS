...
%% geometry is given as a NURBS struct
  ...
  %% ADDITION -------------------------------------------------------------------------------  
  elseif (isstruct(in) && isfield(in, 'form') && strcmpi(in.form, 'Per-BSpline'))
    geometry.perbspline = in;
  %% END ADDITION ----------------------------------------------------------------------------
...
% This allows to use a rectangular parametric domain, instead of [0,1]^n.
  ...
  % ADDITION
  % ------------------------------------------------------------------------------------------
  % If its periodic bslpine use the bspline toolbox
  if (isfield(geometry, 'perbspline'))
    % Check for any entries in each dimension of the coefs and set dim
    % accordingly
%     if (any (abs(geometry.perbspline.coefs(3,:)) > 1e-12))
%         rdim = 3;
    if (any (abs(geometry.perbspline.coefs(2,:)) > 1e-12))
        rdim = 2;
    else
        rdim = 1;
    end
    
    geometry.rdim = rdim; %according to geo_nurbs.m this is the dim of the physical domain
    
%     [deriv, deriv2] = perbspderiv(geometry.perbspline);
%     geometry.dperbspline = deriv;
%     geometry.dperbspline2 = deriv2;
    crv=geometry.perbspline;
    
    geometry.map      =  @(PTS) periodicSplineCurveEval(crv.knots, PTS, crv.order-1, crv.coefs);
    geometry.map_der  =  @(PTS) periodicSplineCurveDerivEval(crv, PTS);
    %geometry.map_der2 =  @(PTS) geo_perbspline (geometry.perbspline, deriv, deriv2, PTS, 2, rdim);
