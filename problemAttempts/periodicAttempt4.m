...
p=3;
crv=periodicCurveInterpolate(9, p, @(t)(cos(2*pi*t)), @(t)(sin(2*pi*t)));
perbspplot(crv,100);
hold on;
crv2=nrbcirc(1);
nrbplot(crv2,100);
...
S=op_f_v_tp(space, msh, @(x,y)(periodicBasisEval(knots,inverseCircle(x,y),basisFun,p)) ); %wont allow constant source?
rhs=(1/delta)*M*c_0+S;
...
%Plot actual result
fplot(@(x)(periodicBasisEval(knots,x,basisFun,p)), [0 1]);