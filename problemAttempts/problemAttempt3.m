%DESCRIPTION: solving for the c in attempt 2 but iterating the time step
...
%set lhs matrix
delta=0.0005; %time step, too big?? too small??
...
%latter doesnt...?
for i=1:stepNumber
    field(i)=nrbmak(transpose(results(:,i)), knots); %results needs to be a row so transpose
end
...
hold on;
for i=1:stepNumber
    [points weights]=nrbeval(field(i), evalPoints);
