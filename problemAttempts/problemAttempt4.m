%DESCRIPTION: solving for the c in attempt 2 but trying to implement
...
for iside = 1:numel(space.boundary)
    drchlet_dofs=union(drchlt_dofs, space.boundary(iside).dofs);
end
int_dofs=setdiff(1:space.ndof, drchlt_dofs);

results=zeros(space.ndof, 1)
%calculate results
results(int_dofs)=mat(int_dofs, int_dofs)\rhs(int_dofs);
